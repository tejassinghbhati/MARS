...
const AGENT_PATH = path.join(__dirname, '..', 'core', 'agent.js');
let runAgent = null;
try {
  runAgent = require(AGENT_PATH).runAgent;
  console.log('🤖 [Scheduler] MARS agent core connected.');
} catch (_) {
  console.warn('⚠️  [Scheduler] Agent core not loadable. Jobs will log prompt instead of calling AI.');
}
